...
const buildNumericalIntegrityTracing = (state, grid) => {
  ...
  return {
    ...
    advectionWaterAddedMassMeanKgM2: round(state.numericalAdvectionWaterAddedMassMeanKgM2 || 0, 7),
    advectionWaterRemovedMassMeanKgM2: round(state.numericalAdvectionWaterRemovedMassMeanKgM2 || 0, 7),
    advectionWaterResidualMassMeanKgM2: round(state.numericalAdvectionWaterResidualMassMeanKgM2 || 0, 7),
    verticalSubtropicalDryingDemandMeanKgM2: round(weightedAccumulationMean(state.verticalSubtropicalDryingDemandMass, nx, ny, latitudesDeg, rowWeights, -90, 90), 7),
    verticalCloudErosionToVaporMeanKgM2: round(weightedAccumulationMean(state.verticalCloudErosionToVaporMass, nx, ny, latitudesDeg, rowWeights, -90, 90), 7),
    northDryBelt: north,
    southDryBelt: south,
    levelBands,
    asymmetry,
...
export function buildValidationDiagnostics(core, { pressureLevelsPa = DEFAULT_PRESSURE_LEVELS_PA } = {}) {
  ...
  return {
    ...
    precipConvectiveRateMmHr: Array.from(state.precipConvectiveRate || []),
    precipStratiformRateMmHr: Array.from(state.precipStratiformRate || []),
    precipAccumMm: Array.from(state.precipAccum || []),
    precipConvectiveAccumMm: Array.from(state.precipConvectiveAccum || []),
    precipStratiformAccumMm: Array.from(state.precipStratiformAccum || []),
    ...
    surfaceNetFluxWm2: Array.from(state.surfaceNetFlux || []),
    landEnergyTempTendencyK: Array.from(state.landEnergyTempTendency || []),
    landClimoTempTendencyK: Array.from(state.landClimoTempTendency || []),
    landHeatCapacityJm2K: Array.from(state.landHeatCapacity || []),
    soilWaterKgM2: Array.from(state.soilW || []),
    soilCapacityKgM2: Array.from(state.soilCap || []),
    soilMoistureFraction: Array.from(state.soilMoistureFraction || []),
    vegetationProxyFrac: Array.from(state.vegetationProxy || []),
    rainforestCanopySupportFrac: Array.from(state.rainforestCanopySupport || []),
    rainforestRootZoneRechargeRateMmHr: Array.from(state.rainforestRootZoneRechargeRate || []),
    oceanMixedLayerTempTendencyK: Array.from(state.oceanMixedLayerTempTendency || []),
    oceanClimoTempTendencyK: Array.from(state.oceanClimoTempTendency || []),
    seaIceThermoTendencyM: Array.from(state.seaIceThermoTendency || []),
    surfaceTemperatureK: Array.from(state.Ts || []),
    surfaceAlbedo: Array.from(state.albedo || []),
    surfaceElevationM: Array.from(core.geo?.elev || []),
    ...
    frontalAscentSupportDiagFrac: arrayOrZeros(state.frontalAscentSupportDiag, state.N),
    frontalAscentAddedDiagPaS: arrayOrZeros(state.frontalAscentAddedDiag, state.N),
    frontalAscentCompensationDiagPaS: arrayOrZeros(state.frontalAscentCompensationDiag, state.N),
    frontalBaroclinicSupportDiagFrac: arrayOrZeros(state.frontalBaroclinicSupportDiag, state.N),
    frontalJetSupportDiagFrac: arrayOrZeros(state.frontalJetSupportDiag, state.N),
    frontalLandOceanSupportDiagFrac: arrayOrZeros(state.frontalLandOceanSupportDiag, state.N),
    frontalMoistureSupportDiagFrac: arrayOrZeros(state.frontalMoistureSupportDiag, state.N),
    stormGenesisPotentialDiagFrac: arrayOrZeros(state.stormGenesisPotentialDiag, state.N),
    stormDeepeningPotentialDiagFrac: arrayOrZeros(state.stormDeepeningPotentialDiag, state.N),
    stormOcclusionPotentialDiagFrac: arrayOrZeros(state.stormOcclusionPotentialDiag, state.N),
    stormDecayPotentialDiagFrac: arrayOrZeros(state.stormDecayPotentialDiag, state.N),
    stormPrecipShieldDiagFrac: arrayOrZeros(state.stormPrecipShieldDiag, state.N),
    stormWarmSectorDiagFrac: arrayOrZeros(state.stormWarmSectorDiag, state.N),
    stormColdSectorDiagFrac: arrayOrZeros(state.stormColdSectorDiag, state.N),
    tropicalCycloneGenesisPotentialFrac: arrayOrZeros(state.tropicalCycloneGenesisPotentialDiag, state.N),
    tropicalCycloneEmbeddedVortexPotentialFrac: arrayOrZeros(state.tropicalCycloneEmbeddedVortexDiag, state.N),
    tropicalCycloneShearSupportFrac: arrayOrZeros(state.tropicalCycloneShearSupportDiag, state.N),
    tropicalCycloneHumiditySupportFrac: arrayOrZeros(state.tropicalCycloneHumiditySupportDiag, state.N),
    tropicalCycloneVorticitySupportFrac: arrayOrZeros(state.tropicalCycloneVorticitySupportDiag, state.N),
    tropicalCycloneBasinSeasonSupportFrac: arrayOrZeros(state.tropicalCycloneBasinSeasonSupportDiag, state.N),
    tornadoRiskPotentialFrac: arrayOrZeros(state.tornadoRiskPotentialDiag, state.N),
    tornadoInstabilitySupportFrac: arrayOrZeros(state.tornadoInstabilitySupportDiag, state.N),
    tornadoShearSupportFrac: arrayOrZeros(state.tornadoShearSupportDiag, state.N),
    tornadoLiftSupportFrac: arrayOrZeros(state.tornadoLiftSupportDiag, state.N),
    tornadoStormModeSupportFrac: arrayOrZeros(state.tornadoStormModeSupportDiag, state.N),
    subtropicalSubsidenceDryingFrac: arrayOrZeros(state.subtropicalSubsidenceDrying, state.N),
    ...
    hadleyReturnFlowWindSupportDiagFrac: arrayOrZeros(state.hadleyReturnFlowWindSupportDiag, state.N),
    hadleyReturnFlowWindAppliedDiagMs: arrayOrZeros(state.hadleyReturnFlowWindAppliedDiag, state.N),
    walkerLongitudinalSubsidenceSupportDiagFrac: arrayOrZeros(state.walkerLongitudinalSubsidenceSupportDiag, state.N),
    ...
    saturationAdjustmentSoftLiveGateRetainedVaporMassKgM2: arrayOrZeros(state.saturationAdjustmentSoftLiveGateRetainedVaporMass, state.N),
    saturationAdjustmentSoftLiveGateSinkExportMassKgM2: arrayOrZeros(state.saturationAdjustmentSoftLiveGateSinkExportMass, state.N),
    saturationAdjustmentSoftLiveGateBufferedRainoutMassKgM2: arrayOrZeros(state.saturationAdjustmentSoftLiveGateBufferedRainoutMass, state.N),
    saturationAdjustmentSoftLiveGateEquatorwardExportMassKgM2: arrayOrZeros(state.saturationAdjustmentSoftLiveGateEquatorwardExportMass, state.N),
