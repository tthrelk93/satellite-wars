...
import { buildPhase1ZXNorthSourceConcentrationPatchDesign } from '../../scripts/agent/phase1zx-north-source-concentration-patch-design.mjs';

test('phase 1ZX prefers a live-leak-signal-driven source concentration cap over humidity or amplitude retunes', () => {
  ...
  const summary = buildPhase1ZXNorthSourceConcentrationPatchDesign({
    ...
    paths: {
      ...
      reportPath: '/tmp/phase1zx.md',
      jsonPath: '/tmp/phase1zx.json'
    }
  });

  assert.equal(summary.verdict, 'live_leak_signal_driven_source_cap_preferred');
  assert.equal(summary.keepWeakHemiFloorTaper, true);
  assert.equal(summary.nextPhase, 'Phase 1ZY: Implement Capped North Source Concentration Penalty Patch');
  assert.equal(summary.ranking[0].name, 'northSourceConcentrationPenalty');
  assert.ok(summary.candidateScores.northSourceConcentrationPenalty > summary.candidateScores.polewardExportPreservingBridge);
  assert.ok(summary.candidateScores.localHumidityOmegaBoost < 0);
});
