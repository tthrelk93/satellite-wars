...
import {
  apply26p25ReceiverCarryoverContainmentTransitionBandPatch,
  classifyC54Decision,
  renderArchitectureC54Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c54-26p25-receiver-carryover-containment-transition-band-experiment.mjs';

test('classifyC54Decision marks quick rejects when severe regressions remain', () => {
  ...
  assert.equal(decision.nextMove, 'Architecture C55: 26p25 receiver carryover containment transition-band attribution');
});

test('renderArchitectureC54Markdown includes the carryover-containment contract', () => {
  const markdown = renderArchitectureC54Markdown({
    decision: {
    ...
    patchSummary: {
      ...
      patchedParams: ['vertical5.receiverCarryoverContainment26p25MaxFrac']
    }
  });

  assert.match(markdown, /Architecture C54 26p25 Receiver Carryover Containment Transition-Band Experiment/);
  assert.match(markdown, /carryover/i);
  assert.match(markdown, /26\.25°/);
  assert.match(markdown, /quick gate pass: false/);
});

test('apply26p25ReceiverCarryoverContainmentTransitionBandPatch contains overlap in the 26.25 receiver lane on top of the live transition-band restore', async (t) => {
  const tmpDir = t.testDir ? t.testDir() : await fs.promises.mkdtemp('/tmp/c54-carry-');
...
`);
  ...
  const summary = apply26p25ReceiverCarryoverContainmentTransitionBandPatch(tmpDir);
  ...
  assert.match(patchedVertical, /containedOverlap/);
  assert.match(patchedVertical, /overlap \* 0\.35 \* receiverCarryoverContainment26p25Frac/);
  assert.match(patchedVertical, /state\.receiverCarryoverContainment26p25Diag/);
  assert.ok(summary.patchedParams.includes('vertical5.receiverCarryoverContainment26p25MaxFrac'));
});
