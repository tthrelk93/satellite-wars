...
import {
  applyStronger26p25ReceiverCarryoverContainmentWith33p75PolewardShoulderCarryoverContainment35degInterfaceEddySofteningAndNarrowerEquatorialCoreGuardPatch,
  classifyC68Decision,
  renderArchitectureC68Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c68-stronger-26p25-receiver-carryover-containment-with-33p75-poleward-shoulder-carryover-containment-35deg-interface-eddy-softening-and-narrower-equatorial-core-guard-experiment.mjs';

test('classifyC68Decision marks quick rejects when severe regressions remain', () => {
  ...
  assert.equal(decision.nextMove, 'Architecture C69: stronger 26p25 receiver carryover containment with 33p75 poleward shoulder carryover containment, 35deg interface eddy softening, and narrower equatorial-core guard attribution');
});

test('renderArchitectureC68Markdown includes the narrower equatorial-core guard contract', () => {
  const markdown = renderArchitectureC68Markdown({
    decision: {
    ...
    patchSummary: {
      ...
      patchedParams: ['windEddyParams.narrowerEquatorialCoreGuardLat1Deg']
    }
  });

  assert.match(markdown, /Architecture C68 Stronger 26p25 Receiver Carryover Containment With 33p75 Poleward Shoulder Carryover Containment 35deg Interface Eddy Softening And Narrower Equatorial-Core Guard Experiment/);
  assert.match(markdown, /narrower equatorial-core guard/i);
  assert.match(markdown, /quick gate pass: false/);
});

test('applyStronger26p25ReceiverCarryoverContainmentWith33p75PolewardShoulderCarryoverContainment35degInterfaceEddySofteningAndNarrowerEquatorialCoreGuardPatch narrows the C66 equatorial guard from 8deg to 6deg', async (t) => {
  const tmpDir = t.testDir ? t.testDir() : await fs.promises.mkdtemp('/tmp/c68-guard-');
...
`);
  ...
  const summary = applyStronger26p25ReceiverCarryoverContainmentWith33p75PolewardShoulderCarryoverContainment35degInterfaceEddySofteningAndNarrowerEquatorialCoreGuardPatch(tmpDir);
  ...
  assert.ok(summary.patchedParams.includes('windEddyParams.narrowerEquatorialCoreGuardLat1Deg'));
});
