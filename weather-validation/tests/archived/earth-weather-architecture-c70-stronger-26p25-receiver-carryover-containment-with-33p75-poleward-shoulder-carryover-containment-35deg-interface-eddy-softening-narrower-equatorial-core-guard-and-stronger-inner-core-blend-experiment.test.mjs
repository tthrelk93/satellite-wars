...
import {
  applyStronger26p25ReceiverCarryoverContainmentWith33p75PolewardShoulderCarryoverContainment35degInterfaceEddySofteningNarrowerEquatorialCoreGuardAndStrongerInnerCoreBlendPatch,
  classifyC70Decision,
  renderArchitectureC70Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c70-stronger-26p25-receiver-carryover-containment-with-33p75-poleward-shoulder-carryover-containment-35deg-interface-eddy-softening-narrower-equatorial-core-guard-and-stronger-inner-core-blend-experiment.mjs';

test('classifyC70Decision marks quick rejects when severe regressions remain', () => {
  ...
  assert.equal(decision.nextMove, 'Architecture C71: stronger 26p25 receiver carryover containment with 33p75 poleward shoulder carryover containment, 35deg interface eddy softening, narrower equatorial-core guard, and stronger inner-core blend attribution');
});

test('renderArchitectureC70Markdown includes the stronger inner-core blend contract', () => {
  const markdown = renderArchitectureC70Markdown({
    decision: {
    ...
    patchSummary: {
      ...
      patchedParams: ['windEddyParams.strongerInnerCoreBlendToUnityFrac']
    }
  });

  assert.match(markdown, /Architecture C70 Stronger 26p25 Receiver Carryover Containment With 33p75 Poleward Shoulder Carryover Containment 35deg Interface Eddy Softening Narrower Equatorial-Core Guard And Stronger Inner-Core Blend Experiment/);
  assert.match(markdown, /stronger inner-core blend/i);
  assert.match(markdown, /quick gate pass: false/);
});

test('applyStronger26p25ReceiverCarryoverContainmentWith33p75PolewardShoulderCarryoverContainment35degInterfaceEddySofteningNarrowerEquatorialCoreGuardAndStrongerInnerCoreBlendPatch strengthens the preserved C68 inner-core blend from 0.3 to 0.4', async (t) => {
  const tmpDir = t.testDir ? t.testDir() : await fs.promises.mkdtemp('/tmp/c70-blend-');
...
`);
  ...
  const summary = applyStronger26p25ReceiverCarryoverContainmentWith33p75PolewardShoulderCarryoverContainment35degInterfaceEddySofteningNarrowerEquatorialCoreGuardAndStrongerInnerCoreBlendPatch(tmpDir);
  const patchedCore = await fs.promises.readFile(corePath, 'utf8');
  const patchedEddy = await fs.promises.readFile(eddyPath, 'utf8');

  assert.match(patchedCore, /equatorialSofteningLat1Deg: 6/);
  assert.match(patchedCore, /equatorialBlendToUnityFrac: 0\.4/);
  assert.match(patchedEddy, /equatorialSofteningLat1Deg = 6/);
  assert.match(patchedEddy, /equatorialBlendToUnityFrac = 0\.4/);
  assert.ok(summary.patchedParams.includes('windEddyParams.strongerInnerCoreBlendToUnityFrac'));
});
