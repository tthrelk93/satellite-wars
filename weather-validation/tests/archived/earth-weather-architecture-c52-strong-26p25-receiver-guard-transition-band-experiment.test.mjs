...
import {
  applyStrong26p25ReceiverGuardTransitionBandPatch,
  classifyC52Decision,
  renderArchitectureC52Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c52-strong-26p25-receiver-guard-transition-band-experiment.mjs';

test('classifyC52Decision marks quick rejects when severe regressions remain', () => {
  ...
  assert.equal(decision.nextMove, 'Architecture C53: strong 26p25 receiver-guard transition-band attribution');
});

test('renderArchitectureC52Markdown includes the strong receiver-guard contract', () => {
  const markdown = renderArchitectureC52Markdown({
    decision: {
    ...
    patchSummary: {
      ...
      patchedParams: ['vertical5.transitionBandStrong26p25ReceiverGuardPenalty']
    }
  });

  assert.match(markdown, /Architecture C52 Strong 26p25 Receiver-Guard Transition-Band Experiment/);
  assert.match(markdown, /26\.25°/);
  assert.match(markdown, /quick gate pass: false/);
});

test('applyStrong26p25ReceiverGuardTransitionBandPatch pulls 26.25 back toward the C32 cap while leaving the outer band live', async (t) => {
  const tmpDir = t.testDir ? t.testDir() : await fs.promises.mkdtemp('/tmp/c52-carry-');
...
`);
  ...
  const summary = applyStrong26p25ReceiverGuardTransitionBandPatch(tmpDir);
  ...
  assert.match(patchedVertical, /strongReceiverGuard26p25Frac/);
  assert.match(patchedVertical, /guardedTransitionBandOrganizedSupportRestoreMax = 0.47/);
  assert.match(patchedVertical, /strongReceiverGuard26p25Frac \* Math\.max\(0, 0\.47 - carryInputOrganizedSupportMax\)/);
  assert.ok(summary.patchedParams.includes('vertical5.transitionBandStrong26p25ReceiverGuardPenalty'));
});
