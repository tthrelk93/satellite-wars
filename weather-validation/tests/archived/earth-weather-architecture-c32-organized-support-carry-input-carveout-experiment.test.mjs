...
import {
  applyOrganizedSupportCarryInputCarveoutPatch,
  classifyC32Decision,
  renderArchitectureC32Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c32-organized-support-carry-input-carveout-experiment.mjs';

test('classifyC32Decision marks quick rejects when severe regressions remain', () => {
  ...
  assert.equal(decision.nextMove, 'Architecture C33: organized-support carry-input carveout attribution');
});

test('renderArchitectureC32Markdown includes the organized-support carveout contract', () => {
  const markdown = renderArchitectureC32Markdown({
    decision: {
    ...
    patchSummary: {
      ...
      patchedParams: ['vertical5.organizedSupportCarryInputCarveoutMax']
    }
  });

  assert.match(markdown, /Architecture C32 Organized-Support Carry-Input Carveout Experiment/);
  assert.match(markdown, /organized equatorial cells/);
  assert.match(markdown, /quick gate pass: false/);
});

test('applyOrganizedSupportCarryInputCarveoutPatch restores stricter caps on top of C30', async (t) => {
...
`);
  const summary = applyOrganizedSupportCarryInputCarveoutPatch(tmpDir);
  ...
  assert.ok(summary.patchedParams.includes('vertical5.organizedSupportCarryInputCarveoutMax'));
  assert.ok(summary.patchedParams.includes('vertical5.potentialCarryInputCarveoutMax'));
});
