...
import {
  applyPartial26p25ReceiverGuardTransitionBandPatch,
  classifyC50Decision,
  renderArchitectureC50Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c50-partial-26p25-receiver-guard-transition-band-experiment.mjs';

test('classifyC50Decision marks quick rejects when severe regressions remain', () => {
  ...
  assert.equal(decision.nextMove, 'Architecture C51: partial 26p25 receiver-guard transition-band attribution');
});

test('renderArchitectureC50Markdown includes the receiver-guard transition-band contract', () => {
  const markdown = renderArchitectureC50Markdown({
    decision: {
    ...
    patchSummary: {
      ...
      patchedParams: ['vertical5.transitionBand26p25ReceiverGuardPenalty']
    }
  });

  assert.match(markdown, /Architecture C50 Partial 26p25 Receiver-Guard Transition-Band Experiment/);
  assert.match(markdown, /26\.25°/);
  assert.match(markdown, /quick gate pass: false/);
});

test('applyPartial26p25ReceiverGuardTransitionBandPatch keeps transition-band restore but guards 26.25 on top of C32', async (t) => {
  const tmpDir = t.testDir ? t.testDir() : await fs.promises.mkdtemp('/tmp/c50-carry-');
...
`);
  ...
  const summary = applyPartial26p25ReceiverGuardTransitionBandPatch(tmpDir);
  ...
  assert.match(patchedVertical, /receiverGuard26p25Frac/);
  assert.match(patchedVertical, /guardedTransitionBandOrganizedSupportRestoreMax = 0.47/);
  assert.match(patchedVertical, /receiverGuard26p25Frac \* 0.025/);
  assert.ok(summary.patchedParams.includes('vertical5.transitionBand26p25ReceiverGuardPenalty'));
});
