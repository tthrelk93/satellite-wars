...
import {
  applyInnerCoreOrganizedSupportRestorePatch,
  classifyC38Decision,
  renderArchitectureC38Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c38-inner-core-organized-support-restore-experiment.mjs';

test('classifyC38Decision marks quick rejects when severe regressions remain', () => {
  const decision = classifyC38Decision({
    quickGatePass: false,
    annualGatePass: null
  });

  assert.equal(decision.verdict, 'quick_reject');
  assert.equal(decision.nextMove, 'Architecture C39: inner-core organized-support restore attribution');
});

test('renderArchitectureC38Markdown includes the inner-core restore contract', () => {
  const markdown = renderArchitectureC38Markdown({
    decision: {
      verdict: 'quick_reject',
      nextMove: 'Architecture C39: inner-core organized-support restore attribution'
    },
    quickRows: [
      {
        label: 'ITCZ width',
    ...
    patchSummary: {
      ...
      patchedParams: ['vertical5.innerCoreOrganizedSupportRestoreMax']
    }
  });

  assert.match(markdown, /Architecture C38 Inner-Core Organized-Support Restore Experiment/);
  assert.match(markdown, /inner equatorial core/);
  assert.match(markdown, /quick gate pass: false/);
});

test('applyInnerCoreOrganizedSupportRestorePatch restores organized support only inside the core on top of C32', async (t) => {
  const tmpDir = t.testDir ? t.testDir() : await fs.promises.mkdtemp('/tmp/c38-carry-');
...
`);
  const summary = applyInnerCoreOrganizedSupportRestorePatch(tmpDir);
  ...
  assert.match(patchedVertical, /innerCoreOrganizedSupportRestoreFrac/);
  assert.match(patchedVertical, /effectiveCarryInputOrganizedSupportMax/);
  assert.match(patchedVertical, /0\.47 - carryInputOrganizedSupportMax/);
  assert.ok(summary.patchedParams.includes('vertical5.innerCoreOrganizedSupportRestoreLat0Deg'));
  assert.ok(summary.patchedParams.includes('vertical5.innerCoreOrganizedSupportRestoreMax'));
});
