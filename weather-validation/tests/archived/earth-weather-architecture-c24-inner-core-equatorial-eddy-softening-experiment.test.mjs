...
import {
  applyInnerCoreEquatorialEddySofteningPatch,
  classifyC24Decision,
  renderArchitectureC24Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c24-inner-core-equatorial-eddy-softening-experiment.mjs';

test('classifyC24Decision marks quick rejects when severe regressions remain', () => {
  ...
  assert.equal(decision.nextMove, 'Architecture C25: inner-core equatorial eddy softening attribution');
});

test('renderArchitectureC24Markdown includes the inner-core softening contract', () => {
  const markdown = renderArchitectureC24Markdown({
    decision: {
    ...
    patchSummary: {
      ...
      patchedParams: ['windEddyParams.innerCoreEquatorialSofteningLat0Deg']
    }
  });

  assert.match(markdown, /Architecture C24 Inner-Core Equatorial Eddy Softening Experiment/);
  assert.match(markdown, /inner-core equatorial eddy-softening quick artifact/);
  assert.match(markdown, /quick gate pass: false/);
});

test('applyInnerCoreEquatorialEddySofteningPatch narrows the C22 band into the inner equatorial core', async (t) => {
  const tmpDir = t.testDir ? t.testDir() : await fs.promises.mkdtemp('/tmp/c24-core-');
...
`);
  const summary = applyInnerCoreEquatorialEddySofteningPatch(tmpDir);
  ...
  assert.match(patchedCore, /equatorialSofteningLat1Deg: 10/);
  assert.match(patchedCore, /equatorialBlendToUnityFrac: 0.45/);
  assert.match(patchedEddy, /equatorialSofteningLat0Deg = 0/);
  assert.match(patchedEddy, /equatorialSofteningLat1Deg = 10/);
  assert.match(patchedEddy, /equatorialBlendToUnityFrac = 0.45/);
  assert.ok(summary.patchedParams.includes('windEddyParams.innerCoreEquatorialBlendToUnityFrac'));
});
