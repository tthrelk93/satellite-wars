...
import {
  apply26p25CarryoverContainmentWith18p75TransitionCarryInputPreservePatch,
  classifyC58Decision,
  renderArchitectureC58Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c58-26p25-carryover-containment-with-18p75-transition-carry-input-preserve-experiment.mjs';

test('classifyC58Decision marks quick rejects when severe regressions remain', () => {
  ...
  assert.equal(decision.nextMove, 'Architecture C59: 26p25 carryover containment with 18p75 transition carry-input preserve attribution');
});

test('renderArchitectureC58Markdown includes the broader transition carry-input preserve contract', () => {
  const markdown = renderArchitectureC58Markdown({
    decision: {
    ...
    patchSummary: {
      ...
      patchedParams: ['vertical5.transitionCarryInputPreserve18p75PotentialMax']
    }
  });

  assert.match(markdown, /Architecture C58 26p25 Carryover Containment With 18p75 Transition Carry-Input Preserve Experiment/);
  assert.match(markdown, /transition carry-input preserve/i);
  assert.match(markdown, /quick gate pass: false/);
});

test('apply26p25CarryoverContainmentWith18p75TransitionCarryInputPreservePatch relaxes the local transition carry-input contract', async (t) => {
  const tmpDir = t.testDir ? t.testDir() : await fs.promises.mkdtemp('/tmp/c58-carry-');
...
`);
  ...
  const summary = apply26p25CarryoverContainmentWith18p75TransitionCarryInputPreservePatch(tmpDir);
  ...
  assert.match(patchedVertical, /transitionCarryInputPreserve18p75Frac/);
  assert.match(patchedVertical, /effectiveCarryInputPotentialMax/);
  assert.match(patchedVertical, /effectiveCarryInputOrganizedSupportMax/);
  assert.match(patchedVertical, /effectiveCarryInputSubtropicalSuppressionMin/);
  assert.match(patchedVertical, /effectiveCarryInputDominanceMin/);
  assert.match(patchedVertical, /effectiveCarryInputMinResidualMassKgM2/);
  assert.match(patchedVertical, /carryInputDominance < effectiveCarryInputDominanceMin/);
  assert.ok(summary.patchedParams.includes('vertical5.transitionCarryInputPreserve18p75PotentialMax'));
});
