...
import {
  applyTaperedPolewardShoulderOrganizedSupportRestorePatch,
  classifyC48Decision,
  renderArchitectureC48Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c48-tapered-poleward-shoulder-organized-support-restore-experiment.mjs';

test('classifyC48Decision marks quick rejects when severe regressions remain', () => {
  ...
  assert.equal(decision.nextMove, 'Architecture C49: tapered poleward-shoulder organized-support restore attribution');
});

test('renderArchitectureC48Markdown includes the tapered poleward shoulder contract', () => {
  const markdown = renderArchitectureC48Markdown({
    decision: {
    ...
    patchSummary: {
      ...
      patchedParams: ['vertical5.taperedPolewardShoulderOrganizedSupportRestoreMax']
    }
  });

  assert.match(markdown, /Architecture C48 Tapered Poleward-Shoulder Organized-Support Restore Experiment/);
  assert.match(markdown, /26.25°/);
  assert.match(markdown, /33.75°/);
  assert.match(markdown, /quick gate pass: false/);
});

test('applyTaperedPolewardShoulderOrganizedSupportRestorePatch keeps 26.25 full while tapering 33.75 on top of C32', async (t) => {
  const tmpDir = t.testDir ? t.testDir() : await fs.promises.mkdtemp('/tmp/c48-carry-');
...
`);
  ...
  const summary = applyTaperedPolewardShoulderOrganizedSupportRestorePatch(tmpDir);
  ...
  assert.match(patchedVertical, /taperedPolewardShoulderOrganizedSupportRestoreFrac/);
  assert.match(patchedVertical, /smoothstep\(24, 26, latAbs\) \* \(1 - smoothstep\(30, 36, latAbs\)\)/);
  assert.match(patchedVertical, /effectiveCarryInputOrganizedSupportMax/);
  assert.ok(summary.patchedParams.includes('vertical5.taperedPolewardShoulderOrganizedSupportRestoreLat0Deg'));
  assert.ok(summary.patchedParams.includes('vertical5.taperedPolewardShoulderOrganizedSupportRestoreMax'));
});
