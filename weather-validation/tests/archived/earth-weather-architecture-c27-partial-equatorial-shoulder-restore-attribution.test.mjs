...
import {
  classifyC27Decision,
  renderArchitectureC27Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c27-partial-equatorial-shoulder-restore-attribution.mjs';

test('classifyC27Decision recognizes upper recovery with lower import reload', () => {
  const decision = classifyC27Decision({
    ...
    c26North35Lower: -22.99819,
    c24North35Mid: -17.16362,
    c26North35Mid: -17.27508,
    c24OceanCond: 0.12705,
    c26OceanCond: 0.11952
  });

  assert.equal(decision.verdict, 'partial_shoulder_restore_recovers_upper_branch_and_return_flow_but_reloads_lower_import_and_cloud_recirculation');
  assert.equal(decision.nextMove, 'Architecture C28: weak partial equatorial shoulder restore experiment');
});

test('renderArchitectureC27Markdown includes the weak-restore follow-up contract', () => {
  const markdown = renderArchitectureC27Markdown({
    decision: {
    ...
    dryBeltComparison: {
      ...
      c26UpperCloudPath: 0.25387,
      c24CloudRecirc: 0.49385,
      c26CloudRecirc: 0.60796,
      c24NorthReturn: 3447.36194,
      c26NorthReturn: 3383.23239,
      c24TransitionFlux: -190.46264,
      c26TransitionFlux: -207.34044,
      c24DryBeltFlux: -414.35421,
      c26DryBeltFlux: -417.17261
    },
    ...
    nextContract: {
      focusTargets: [
        'retain only a weaker shoulder restoration than C26'
      ]
    }
  });

  assert.match(markdown, /Architecture C27 Partial Equatorial Shoulder Restore Attribution/);
  assert.match(markdown, /weaker shoulder restoration than C26/);
  assert.match(markdown, /cloud recirculation proxy/);
});
