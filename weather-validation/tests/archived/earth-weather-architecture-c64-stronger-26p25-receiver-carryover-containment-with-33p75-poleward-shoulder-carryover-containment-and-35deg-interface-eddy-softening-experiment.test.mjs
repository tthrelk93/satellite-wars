...
import {
  applyStronger26p25ReceiverCarryoverContainmentWith33p75PolewardShoulderCarryoverContainmentAnd35degInterfaceEddySofteningPatch,
  classifyC64Decision,
  renderArchitectureC64Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c64-stronger-26p25-receiver-carryover-containment-with-33p75-poleward-shoulder-carryover-containment-and-35deg-interface-eddy-softening-experiment.mjs';

test('classifyC64Decision marks quick rejects when severe regressions remain', () => {
  ...
  assert.equal(decision.nextMove, 'Architecture C65: stronger 26p25 receiver carryover containment with 33p75 poleward shoulder carryover containment and 35deg interface eddy softening attribution');
});

test('renderArchitectureC64Markdown includes the 35deg interface eddy-softening contract', () => {
  const markdown = renderArchitectureC64Markdown({
    decision: {
    ...
    patchSummary: {
      ...
      patchedParams: ['windEddyParams.enable35degInterfaceEddySoftening']
    }
  });

  assert.match(markdown, /Architecture C64 Stronger 26p25 Receiver Carryover Containment With 33p75 Poleward Shoulder Carryover Containment And 35deg Interface Eddy Softening Experiment/);
  assert.match(markdown, /35deg interface eddy-softening/i);
  assert.match(markdown, /quick gate pass: false/);
});

test('applyStronger26p25ReceiverCarryoverContainmentWith33p75PolewardShoulderCarryoverContainmentAnd35degInterfaceEddySofteningPatch adds a narrow interface eddy-softening lane on top of C62', async (t) => {
  const tmpDir = t.testDir ? t.testDir() : await fs.promises.mkdtemp('/tmp/c64-eddy-');
...
`);
  ...
  const summary = applyStronger26p25ReceiverCarryoverContainmentWith33p75PolewardShoulderCarryoverContainmentAnd35degInterfaceEddySofteningPatch(tmpDir);
  const patchedCore = await fs.promises.readFile(corePath, 'utf8');
  const patchedEddy = await fs.promises.readFile(eddyPath, 'utf8');

  assert.match(patchedCore, /enable35degInterfaceEddySoftening: true/);
  assert.match(patchedCore, /interfaceEddyBlendToUnityFrac: 0\.3/);
  assert.match(patchedEddy, /const smoothstep =/);
  assert.match(patchedEddy, /const interfaceShoulderWeight = enable35degInterfaceEddySoftening/);
  assert.ok(summary.patchedParams.includes('windEddyParams.enable35degInterfaceEddySoftening'));
});
