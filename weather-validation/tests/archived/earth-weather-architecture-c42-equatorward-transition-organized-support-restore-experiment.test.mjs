...
import {
  applyEquatorwardTransitionOrganizedSupportRestorePatch,
  classifyC42Decision,
  renderArchitectureC42Markdown
} from '../../../scripts/agent/archived/earth-weather-architecture-c42-equatorward-transition-organized-support-restore-experiment.mjs';

test('classifyC42Decision marks quick rejects when severe regressions remain', () => {
  ...
  assert.equal(decision.nextMove, 'Architecture C43: equatorward-transition organized-support restore attribution');
});

test('renderArchitectureC42Markdown includes the equatorward-transition contract', () => {
  const markdown = renderArchitectureC42Markdown({
    decision: {
    ...
    patchSummary: {
      ...
      patchedParams: ['vertical5.equatorwardTransitionOrganizedSupportRestoreMax']
    }
  });

  assert.match(markdown, /Architecture C42 Equatorward-Transition Organized-Support Restore Experiment/);
  assert.match(markdown, /26.25° receiver lane/);
  assert.match(markdown, /quick gate pass: false/);
});

test('applyEquatorwardTransitionOrganizedSupportRestorePatch narrows the restore away from the 26.25 receiver lane', async (t) => {
  const tmpDir = t.testDir ? t.testDir() : await fs.promises.mkdtemp('/tmp/c42-carry-');
...
`);
  const summary = applyEquatorwardTransitionOrganizedSupportRestorePatch(tmpDir);
  ...
  assert.match(patchedVertical, /equatorwardTransitionOrganizedSupportRestoreFrac/);
  assert.match(patchedVertical, /smoothstep\(12, 18, latAbs\) \* \(1 - smoothstep\(22, 26, latAbs\)\)/);
  assert.match(patchedVertical, /effectiveCarryInputOrganizedSupportMax/);
  assert.ok(summary.patchedParams.includes('vertical5.equatorwardTransitionOrganizedSupportRestoreLat0Deg'));
  assert.ok(summary.patchedParams.includes('vertical5.equatorwardTransitionOrganizedSupportRestoreMax'));
});
